...
<script lang="ts">
  ...
  async function loadTransactions() {
    try {
      ...
      console.log('Loading transactions for:', $walletStore.address);
      
      // Get transactions from local storage instead of EtherScan
      const txHistory = await walletService.getTransactionHistory();
      
      console.log('Transaction history from local storage:', txHistory);
      ...
      console.log('Processing', txHistory.length, 'transactions');
      
      const ethPrice = await walletService.getCurrETHPrice();
      
      transactions = txHistory.map((tx: any, index: number) => {
        // Value is stored in wei as a string, convert to ETH
        const amountInEth = parseFloat(tx.value || '0') / 1e18;
        ...
        const isSent = tx.from.toLowerCase() === $walletStore.address!.toLowerCase();
  ...
  <div class="transactions-container">
    ...
    {:else if transactions.length === 0}
      <div class="empty-state">
        ...
        <p class="empty-text">No transactions yet</p>
        <p class="empty-subtext">Transactions made through VSWallet will appear here</p>
      </div>
    {:else}
      <div class="transactions-list">
        {#each transactions as transaction}
            ...
            <div class="transaction-details">
              ...
              {#if transaction.note}
                <div class="transaction-note">
                  <span class="note-label">Note:</span> {transaction.note}
                </div>
              {/if}
